...
from croco.utils.misc import NativeScalerWithGradNormCount  # noqa

from dust3r.model_multiimage2 import AsymmetricCroCo3DStereo, inf  # noqa: F401, needed when loading the model
...
def load_model(args, device):
    ...
    if args.croco_checkpoint and not args.resume:
        ...
        ckpt = torch.load(args.croco_checkpoint, map_location=device, weights_only=False)
...
def train(args, input_queue, output_queue):
    ...
    # print(optimizer)
    loss_scaler = NativeScalerWithGradNormCount(bool(args.amp))
    ...
    def save_model(epoch, fname, best_so_far=None, best_pose_ate_sofar=None):
    ...
    best_so_far, best_pose_ate_sofar = misc.load_model(args=args, model_without_ddp=model_without_ddp, optimizer=optimizer, loss_scaler=loss_scaler)
...
def train_one_epoch(croco: torch.nn.Module, model_without_ddp: torch.nn.Module, criterion: torch.nn.Module,
    ...
    # for data_iter_step in range(len(data_loader)):
        ...
        true_shape_context = torch.stack([v['true_shape_context'] for v in views]).to(device, non_blocking=True) 
        ...
        batch = {
            ...
            'true_shape': true_shape_context,
            'time_embedding': time_embedding_context,
            'idx': torch.arange(num_views)[None].repeat(batch_size, 1).to(device, non_blocking=True),
        }
        ...
        batch_result = loss_of_one_batch(slice_time(batch, 0, 5), slice_time(labels, 0, 5), croco, criterion, use_amp=bool(args.amp), step=data_iter_step)
...
def test_one_epoch(croco: torch.nn.Module, model_without_ddp: torch.nn.Module, criterion: torch.nn.Module,
